...
### Automatic Alignment
When requesting memory of size **s** and **s** can be expressed as product of two positive integers **s** = **n**\***m** such that **m** takes the highest possible power of 2, then the returned memory is aligned to the lesser of **m** and `TBMAN_ALIGN`. In practice this means that if you allocate an array of data type `my_type` with `sizeof( my_type )` being a power of two smaller-equal to `TBMAN_ALIGN`, then the memory block is alinged to `sizeof( my_type )`. Consequently all elements of the array are aligned to `sizeof( my_type )`. 
...
For large memory requests, where pooling would be wasteful, tbman falls back to using direct system calls. However, it [keeps track](#anchor_memory_tracking) of all memory.
